document.addEventListener('DOMContentLoaded', function() {
            ...
            if (selectedType === 'mix') {
                ...
                addMasteringCheckbox.addEventListener('change', function() {
                    ...
                    if (addMasteringCheckbox.checked) {
                        mixPrice += 25;
                    } else if (!addMasteringCheckbox.checked && mixPrice > (selectedRevision === 'none' ? 75 : selectedRevision === '1-2' ? 100 : 150)) {
                        mixPrice -= 25;
                    }
                    if (over8TracksCheckbox && over8TracksCheckbox.checked) {
                        mixPrice += 25;
                ...
                over8TracksCheckbox.addEventListener('change', function() {
                    ...
                    if (over8TracksCheckbox.checked) {
                        mixPrice += 25;
                    } else if (!over8TracksCheckbox.checked && mixPrice > (selectedRevision === 'none' ? 75 : selectedRevision === '1-2' ? 100 : 150) + (addMasteringCheckbox.checked ? 25 : 0)) {
                        mixPrice -= 25;
                    }
                    priceSpan.textContent = `$${mixPrice} (${selectedRevision === 'none' ? 'no revisions' : selectedRevision === '1-2' ? '1-2 revisions' : 'unlimited revisions'})`;
                    updateTotalPrice();
                ...
                uploadSelect.addEventListener('change', function() {
                    ...
                    } else if (selectedUploadType === 'stems' || selectedUploadType === 'protoolsSession') {
                        linkInput.style.display = 'none';
                        fileInput.style.display = 'block';
                    }
                });
    ...
    document.getElementById('completeButton').addEventListener('mouseout', function() {
    ...
    document.getElementById('completeButton').addEventListener('click', function() {
        ...
        const totalCost = parseFloat(document.getElementById('totalPrice').textContent.replace('Total: $', '').replace('(', '').replace(')', ''));
...
function updateTotalPrice() {
    ...
    if (totalCost >= 500) {
        ...
        discountedAmount = totalCost * 0.20;
        discountText = '(%20 off)';
    ...
    if (discountText) {
        document.getElementById('totalPrice').textContent = `Total: $${totalCost.toFixed(2)} ${discountText} = $${finalAmount.toFixed(2)}`;
    } else {
        document.getElementById('totalPrice').textContent = `Total: $${totalCost.toFixed(2)}`;
    }
}
